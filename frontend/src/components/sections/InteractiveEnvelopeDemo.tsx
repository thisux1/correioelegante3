      ...
      <Container size="default">
        ...
        <div className="text-center mb-14">
          ...
          <p className="text-base sm:text-lg text-[#701a35] font-medium max-w-xl mx-auto">
            Toque no lacre de cera abaixo para ver como quem você ama vai receber sua homenagem.
          </p>
        </div>

        {/* Envelope Interativo */}
        <div className="max-w-2xl mx-auto">
          <CardTilt3D intensity={isOpen ? 4 : 10}>
            <div className="relative rounded-3xl border-2 border-pink-300/80 bg-white shadow-2xl shadow-rose-500/10 p-6 sm:p-10 transition-all">
              
              {!isOpen ? (
                /* Estado Fechado: Envelope com Selo de Cera */
                <motion.div
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="py-12 px-4 sm:px-8 text-center flex flex-col items-center justify-center cursor-pointer select-none"
                  onClick={handleOpenEnvelope}
                >
                  <div className="relative mb-8">
                    {/* Silhueta do envelope fechado */}
                    <div className="w-64 sm:w-80 h-44 sm:h-52 rounded-2xl bg-gradient-to-br from-[#fff1f5] to-[#ffe4ec] border-2 border-pink-300 shadow-md relative overflow-hidden flex items-center justify-center">
                      <div className="absolute inset-0 bg-white/30 backdrop-blur-2xs" />
                      
                      {/* Dobras do envelope */}
                      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 320 200" fill="none">
                        <polygon points="0,0 160,110 320,0" fill="#ffe4ec" stroke="rgba(244,63,94,0.3)" strokeWidth="1.5" />
                        <polygon points="0,200 160,110 0,0" fill="#ffd1dc" opacity="0.4" />
                        <polygon points="320,200 160,110 320,0" fill="#ffd1dc" opacity="0.4" />
                        <polygon points="0,200 320,200 160,110" fill="#fff5f8" stroke="rgba(244,63,94,0.3)" strokeWidth="1.5" />
                      </svg>

                      {/* Selo de Cera 3D Clicável */}
                      <motion.button
                        type="button"
                        whileHover={{ scale: 1.12 }}
                        whileTap={{ scale: 0.92 }}
                        className="relative z-10 w-16 h-16 rounded-full bg-gradient-to-br from-[#f43f5e] via-[#e11d48] to-[#be123c] shadow-xl flex items-center justify-center text-white ring-4 ring-rose-400/30 border border-white/40 cursor-pointer"
                        aria-label="Abrir carta"
                      >
                        <Heart size={24} className="fill-white drop-shadow-xs animate-pulse" />
                      </motion.button>
                    </div>
                  </div>

                  <h3 className="font-display text-2xl font-bold text-[#4c0519] mb-2">
                    Para: Beatriz
                  </h3>
                  <p className="text-sm font-medium text-[#e11d48] flex items-center justify-center gap-1.5 animate-bounce">
                    <Sparkles size={16} />
                    Toque no selo de cera para deslacrar
                  </p>
                </motion.div>
              ) : (
                /* Estado Aberto: Carta Revelada */
                <motion.div
                ...
                >
                  ...
                  {/* Texto da Carta */}
                  <div className="bg-[#fff9fa] p-5 sm:p-6 rounded-2xl border border-pink-200/80">
                  ...
                  {/* Raspadinha de Segredo */}
                  <div className="rounded-2xl border-2 border-pink-200 bg-pink-50/60 p-4 text-center">
                    <AnimatePresence mode="wait">
                      {!isScratched ? (
                        ...
                        >
                          ...
                          <span>Toque para raspar a surpresa da carta</span>
                        </motion.button>
                      ) : (
                        <motion.div
                          key="revealed"
                        ...
                        >
                          <p className="text-xs text-[#701a35] mb-0.5">Surpresa Revelada</p>
                          <p className="text-base font-bold text-[#4c0519]">
                            Passagens reservadas para a nossa viagem a Paris!
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>

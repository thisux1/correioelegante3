      ...
      <Container size="default">
        <header className="mb-8 flex flex-col gap-5 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1.5">
            <h1 className="font-display text-4xl font-bold text-text sm:text-5xl">Analytics</h1>
            <p className="max-w-2xl text-sm leading-relaxed text-text-light sm:text-base">
              Panorama geral da plataforma: crescimento, conteúdo e receita em um só lugar.
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <Card glass className="py-16 text-center">
            ...
            <h2 className="mb-1 font-display text-lg font-bold text-text">Ainda não há dados para exibir</h2>
            <p className="mx-auto mb-6 max-w-sm text-xs text-text-light sm:text-sm">
              Assim que as primeiras cartas forem criadas, os indicadores da plataforma aparecerão aqui.
            </p>
            <Button variant="outline" onClick={handleRetry}>
              <RefreshCw size={16} />
              Atualizar dados
          ...
          <div className="space-y-10">
            ...
            <section aria-label="Séries temporais" className="space-y-6">
              <h2 className="font-display text-2xl font-bold text-text">Movimento diário</h2>
              <div className="grid gap-6 lg:grid-cols-2">
                <ChartCard
                  title="Novos cadastros"
                  description={`Cadastros por dia nos últimos ${period} dias`}
                >
                  <LineChart
                    data={signupSeries}
                    ariaLabel={`Novos cadastros por dia nos últimos ${period} dias`}
                ...
                <ChartCard
                  title="Visualizações"
                  description={`Visualizações de cartas por dia nos últimos ${period} dias`}
                >
                  <LineChart
                    data={viewSeries}
                    ariaLabel={`Visualizações por dia nos últimos ${period} dias`}
                    valueFormatter={formatNumber}
                  />
                </ChartCard>
                ...
                <div className="lg:col-span-2">
                  <ChartCard
                    ...
                    description={`Pagamentos aprovados por dia nos últimos ${period} dias`}
                  >
                    <BarChart
                      data={paymentSeries}
                      orientation="vertical"
                      ariaLabel={`Pagamentos concluídos por dia nos últimos ${period} dias`}
                      valueFormatter={formatNumber}
                    />
                  </ChartCard>
                </div>
            ...
            <section aria-label="Conteúdo criado" className="space-y-6">
              <div className="space-y-1">
                <h2 className="font-display text-2xl font-bold text-text">Conteúdo</h2>
                <p className="text-sm text-text-light">
                  Média de{' '}
                  <strong className="font-semibold text-[#701a35]">
                    {formatNumber(content?.avgBlocksPerPage ?? 0)} blocos por página
                  </strong>{' '}
                  entre todas as páginas publicadas.
                </p>
              </div>
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <ChartCard
                  ...
                  description="Distribuição de uso por tipo de bloco no editor"
                >
                  {(content?.blockTypes.length ?? 0) > 0 ? (
                    <BarChart
                      data={(content?.blockTypes ?? []).map((item) => ({ label: item.type, value: item.count }))}
                      orientation="horizontal"
                      ariaLabel="Distribuição de tipos de bloco"
                      valueFormatter={formatNumber}
                    />
                  ) : (
                ...
                <ChartCard
                  ...
                  description="Temas escolhidos com mais frequência nas cartas"
                >
